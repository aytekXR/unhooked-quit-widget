...
| **ME-9** Paywall polish + goldens | ⬜ open | Phase 4. **READ THIS BEFORE PUTTING A FIELD ON THE PAYWALL.** S54 measured what §6.6's "Waterline backdrop" costs here: the paywall renders BOTH translucent forms — the selected plan card (`primary@12%`) and the failure banner (`themedCautionCard`) — and a translucent fill over a base the field has ALREADY tinted composites through two layers. Its true max safe field opacity is **0.0391, BELOW the field's own standard 0.06**, so a full-bleed field fails light `primary action text on selection tint` (4.387 vs 4.5) immediately. Three honest routes: honour §6.6's "≤ top third" literally and keep the field off the cards; ship at ≤0.039; or **apply S54's opaque-floor pattern** (an opaque `surface/base` pinned beneath each tinted fill — one line per surface, byte-identical where no field exists), which is the one that generalises. Note also that no golden and no audit mount renders the failure banner, so only a harness can check it |
