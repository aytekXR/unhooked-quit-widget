...
| **ME-9** Paywall polish + goldens | ⬜ open — **SCOPED, with the field question already measured** | Phase 4, and the last build item before the golden batch. **THE FIELD: §6.6's "≤ top third" read LITERALLY is both the prettier route and the SAFE one** — over a top band only the header sits on the field, true ceiling **0.2140** (3.5× the field's standard 0.06, safe even past the rejected 12%). A **FULL-BLEED** field is the unsafe reading: it drags in the selected plan card (`primary@12%`) and the failure banner (`themedCautionCard`), both **doubly translucent**, true ceiling **0.0391** — below standard, failing light `primary action text on selection tint` at 4.387 vs 4.5. *(This supersedes the blunter S54 note that said "a field on the paywall is unsafe at standard opacity" without distinguishing the two readings.)* Optionally also pin the opaque-floor pattern under those two fills — byte-identical today, and it stops a future full-bleed change re-opening the hole. **Four things already verified, do not re-derive:** (1) the trial badge is ALREADY `semantic/positive` on a deliberately neutral capsule and the plan cards are ALREADY `radius.l` — UIR-4 shipped both, so "the plan-card visual pass" is mostly done; (2) the "See your plan options" row needs **no new copy and no new row** — `DiscreetSettingsCopy.winbackRowLabel` exists and `yourPlanSection` renders it, merely gated on `winbackEligible`, so ME-9 widens a condition; (3) widening it moves **ZERO settings goldens** (that fixture is repository-LESS and the section takes a repository), so ME-9's whole churn is **4 NEW paywall goldens**; (4) a feathered band crop must live in `DesignSystem/` — `ThemeSourceLintTests` bans `Color.white`/`.black` everywhere else and a gradient mask needs one. The paywall fixture needs no seam and zero RevenueCat symbols. **Nothing here renders the failure banner in any golden or audit mount — only a harness can check that composite.** |
