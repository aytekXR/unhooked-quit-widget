...
| QW-3 Analytics fire-points | 🟡 **2 of 5 (S61)** | `quit_created` (both `createQuit` overloads) and `erase_all_completed` wired to their Architect-assigned seams, with `QuitCreatedWireTests`. **Still dormant: `panic_opened` (+cold-start bucket), `panic_step_reached`, `slip_logged`.** ⚠️ **S61 found WHY, and it re-sizes this row: they are not missing call sites, they are blocked on an architectural constraint nobody had written down.** The panic route is **store-free by contract** (ADR-6, <2s cold launch: it reads only the App Group pre-cache and opens no SwiftData store). Consent lives ONLY in `AppSettings.analyticsOptIn` inside SwiftData and is **never mirrored to the App Group** (grep-verified) — so on the cold panic path there is no way to read consent without opening the store, which is exactly the read the latency budget forbids. A naive `analytics.fire(.panicOpened(…))` in `PanicFlowModel` would therefore either blow ADR-6 or fire un-consented, and the second is an App Privacy violation. **The correct mechanism already exists and is the one to reuse:** `PanicOutcomeBuffer` + `QuitRepository.flushPanicOutcomes()`, which buffers on the cold path and fires `.urgeAverted` *past the commit point* once the store and consent are available. Wiring these three means extending that deferred-fire path (and its durable JSON draft format, so migration care) — **not an "S" of call sites.** `slip_logged` has its own version of the same problem: the 10-minute undo window is client-side in `SlipFlowModel` with no post-window flush at all. |
...
34 WCAG-pinned contrast pairs". Counted from disk and source at S58: **149 goldens across 14
suites** (ME-9 added `PaywallSnapshotTests` and its six), **32 registered contrast pairs** — ME-9
added none either, for the third session running, because a decorative band carries no text and an
opaque floor RESTORES a registered composite rather than creating one — and **11 audited surfaces** (not 8, and no longer the
