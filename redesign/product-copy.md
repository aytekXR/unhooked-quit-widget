...
> **Synced with the §3 founder copy sitting — Session 46, 2026-07-25.** That sitting is the one `design-roadmap.md` schedules as "the longest-lead dependency in the project," and it has now HAPPENED: every shipping copy table was read end-to-end, ~20 decisions were made, and the edits are landed and green (404 unit / 35 snapshot / 121 free-lane; 12 goldens re-recorded). Rows below that this document had marked **Keep** and the sitting nonetheless changed are re-marked **CHANGED — §3 sitting, S46** with the reasoning inline. Four of them are **not taste calls** and must not be reverted: the paywall named a `journal` feature `mvp.md:68` puts out of MVP scope; the quiz promised a weekly-limit editor that does not exist; the win-back said "Your annual plan" to users who may never have paid; and the panic entry title pushed the breath bloom off-screen at max Dynamic Type. The rest are the founder's taste calls, made with the evidence in front of them. The full record is in `docs/past-prompts.md` (Session 46) and `docs/copy-pass-checklist.md`.
...
| 12 | Helper | "You can change this anytime." | **"Adjust until it feels right."** | **CHANGED — §3 sitting, S46.** Not taste: source-verified that NO post-quiz weekly-limit editor exists anywhere in the settings surface, so "change this anytime" promised an unbuilt feature. The replacement keeps the no-pressure spirit. |
| 13 | Title (commitment) | "How ready do you feel?" | "How ready do you feel?" | **Keep.** |
| 13 | Slider echoes | "Taking it slow / Getting there / Ready when you are / Ready to start today" | "Taking it slow / Getting there / **Almost there** / Ready to start today" | **CHANGED — §3 sitting, S46.** Still all judgment-free (that reasoning stands). The third rung was the APP addressing the user ("we're ready when you are") on a step that asks the user to describe their OWN readiness; "Almost there" answers in the user's voice and completes the progression. Note this does NOT touch the widget/empty-state "Ready when you are." — a different string that keeps the cross-surface motif. |
...
| Savings caption | "saved in a year, if you stay on track." | **"saved in a year at this rate."** | **CHANGED — §3 sitting, S46.** The view renders "/year" as a suffix directly ABOVE this caption, so the user read "year" twice in two lines. Still an honest hedge. |
| Savings-absent line | "Every clean day is time and focus back — we'll count the streak that matters to you." | **"Every day you hold is time and focus back — we'll count the streak that matters to you."** | **CHANGED — §3 sitting, S46.** See the "clean" ruling below. |
...
| Positioning line | "No account. No sign-up. Apple handles billing — cancel in one tap." | **"No account. Apple handles billing — cancel in one tap."** | **CHANGED — §3 sitting, S46.** The founder made the call this row anticipated: the mvp §6 "No server" canon stays withheld (it is self-contradictory on the one RevenueCat-brokered screen, and Apple refunds are requested, not one-tap). Additionally "No sign-up" was dropped as pure redundancy — every account requires signing up. |
| Notes line | "Your notes and journal never leave your device." | **"Your notes and reflections never leave your device."** | **CHANGED — §3 sitting, S46. Not taste.** The urge **journal** is a PRD P1 feature that `mvp.md:68` puts explicitly OUT of MVP scope; only slip reflection notes ship in v1.0. A paywall — the screen asking for money — naming a feature the build does not have is both a trust problem and needless 3.1.2 exposure. |
...
| Win-back offer | "Your annual plan, now at half price." | **"Annual plan, now at half price."** | **CHANGED — §3 sitting, S46. Not taste.** "Half price" is indeed honest (a real ASC promotional offer) and stays — that reasoning was never the issue. The problem is **"Your"**: the win-back fires 7 days after a TRIAL lapse, and a trial-lapse user may never have paid, so they have no annual plan to call theirs. This is the exact false-ownership framing the win-back register bans. |
...
| Momentum explainer (tap) | — | "Momentum is your steady days over all your days. A slip barely moves it — only time does." | **New**, with an S46 amendment: "clean" → **"steady"**, per the ruling below. The popover idea stands and is a good one. |
...
| Element | Current | New copy | Notes |
|---|---|---|---|
| Entry title | "Let's take this one wave at a time." | **"Just this one wave."** | **CHANGED — §3 sitting, S46. Not taste.** The AX5 goldens proved the docs' long-standing "it truncates" claim wrong — it wraps — but revealed something worse nobody had recorded: at max Dynamic Type the four-line title pushed the **breath bloom entirely off-screen**, leaving a user at the largest accessibility sizes with only text and no pacer to follow, on the one screen whose whole job is "follow this". The 19-char title takes two lines and the bloom is back in frame. Echoes the timer step's "Just this wave." |
...
> ⚠️ **S46 CONFLICT — the notification tier contradicts a v1.0 ratification.** In the §3 sitting the founder RATIFIED that v1.0 requests **no notification permission at all**: the win-back ships in-app only (`mvp.md` §6), and `docs/review-notes.md` now tells App Review in writing that "the app never requests notification permission" — a claim a reviewer can falsify in one launch. Every row in this notification section is therefore **v1.1-or-later**, not v1.0, unless the founder reverses that ratification. If it is reversed, `review-notes.md` §2 and `mvp.md` §6 must move in the SAME session, before any build ships.
...
All shipped strings **Keep** — lexicon-gated and golden-pinned; listed so sibling documents quote them correctly. Rectangular widget: "Day 34" / "$412 saved" / panic button (discreet: money dropped, label "Reset"). Unavailable state: "Ready when you are." Gallery: "Streak — Your streak, on your lock screen. A quick reset is one tap away." Control Center: "Panic — Opens a full-screen reset, instantly." (**S46 note:** the WIDGET's VoiceOver label moved to "Panic — opens a 90-second urge exercise" per brandkit §8, which the shipped string had silently drifted from; these three system-UI intent strings were deliberately NOT changed with it because the operator's §7 device matrix checks them verbatim — aligning them is an open one-line call in `operator-expected.md` §3) and "Reset — Opens a quick reset." Alternate icons: "Calendar style" / "Timer style". App-switcher shield: no text by design — a blank card draws no eye.
