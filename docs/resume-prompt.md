...
>
> S61 wrote a hypothesis down, measured it in the same session, and **refuted it** —
> which is why the geometry is now established rather than argued. The claim was
> *"the accessibility frame runs past the bottom of the WINDOW, so `.contrast`
> samples the black beyond it."* Run `30718997823` answered:
>
> ```
...
> ```
>
> **What that bought.** 807.3 and 754.7 are the same numbers measured off the audit's
> own PNG crops, so **the crops ARE the element frames** — established, not inferred.
> And a paragraph whose visible glyphs occupy roughly the top 90pt reports a frame of
> **807pt on an 874pt window, 92% of the screen** (the quiz's is 86%). The comparison
> was against the wrong rectangle: the viewport ends where the pinned action zone
> begins, far above 874pt, so a frame that large **necessarily overlaps the picker
> and the CTA**. That explains a spurious contrast sample AND is a real defect of a
> different kind — an element whose accessibility frame swallows the controls beneath
> it is a VoiceOver-focus and hit-region problem.
>
> **The probe now measures the RIGHT rectangle** (`exceedsViewport`,
> `overhangPastViewport`, and the frame's y-origin against the ScrollView's), so your
> first run prints the answer without you designing anything. Read it with
> `gh run view <id> --log | grep "R61.1\["`. If the overhang is real, the fix is
> about the element's FRAME, not its colour — and it belongs to whatever puts a
> paragraph's accessibility frame outside its scroll viewport.
>
...
>
> **(A) R61.1, AND HALF THE DIAGNOSIS IS ALREADY DONE — read it before adding to it.** S61
> wrote a hypothesis down, measured it, and REFUTED it: the frames do NOT run past the window
> (`ageGate.entry` 807.3pt and `quiz.consent` 754.7pt against an 874pt window). What that
> bought is worth more than the guess would have been — those are the same numbers measured
> off the audit's own PNG crops, so **the crops ARE the element frames**, and a paragraph whose
> visible glyphs occupy the top ~90pt reports a frame covering **92% of the screen**. The
> comparison was against the wrong rectangle: the viewport ends where the pinned action zone
> begins, so a frame that large **necessarily overlaps the picker and the CTA** — which
> explains a spurious contrast sample AND is a real VoiceOver-focus/hit-region defect in its
> own right. **The probe now measures against the SCROLLVIEW**, so your first run prints the
> answer: `gh run view <id> --log | grep "R61.1\["`.
>
