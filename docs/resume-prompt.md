...
> **You are NOT blocked. Three objectives, in this order: (A) R58.2 — the last of the two defects
> ME-9's goldens exposed — then (B) QW-6, the crest mark's first in-app appearance, then (C) the
> final golden batch.** (B) before (C) is a dependency, not a preference: the batch's remaining scope
> IS the age gate, and QW-6 rewrites the age gate.
>
> **R58.1 IS ALREADY FIXED. Read how before you touch `PaywallView`, because the obvious fix is the
> one that was measured and rejected.** Pinning `statusSurface` into the footer zone truncated that
> footer at AX5 to "Restore purch…", "Terms…", "Privacy…" (run `30509129672`). What shipped keeps the
> banner in the scroll and SCROLLS TO IT (`ScrollViewReader` + `.onChange(of: model.phase)`), so the
> pinned zone is untouched by construction; it is gated by a UI test asserting `retry.isHittable`,
> **not `exists`** — the banner always existed, off-screen. **The `paywall_failed` goldens still show
> it off-screen and that is correct, not a regression:** the fixture drives the failure before
> `assertSnapshot` renders, so the view is born at `.failed` and the golden captures frame zero.
> Those goldens pin COMPOSITION. Do not re-fix a fixed defect.
>
> **(A) R58.2, and isolate it into its own run.** On the TEASER arm the 3.1.2(c) auto-renewal
> disclosure clips mid-sentence, because the taller teaser footer squeezes the viewport —
> `PaywallView.swift:50-59` asserts it "renders ON the screen, before any purchase" and on that arm
> it does not. It matters because §8 gives the REVIEW BUILD to that arm. Pre-existing since E7.2
> (S25); invisible until the paywall had goldens. **Layout claims are billed-run questions on this
> box** — `swiftc -parse` cannot see layout, and `test_a11yAudit_paywall_noViolations` mounts at the
> DEFAULT content size, so only a golden sees AX5 here. Expect the two `teaser` goldens to re-record;
> movement in the other four means your footer assumption was wrong. **There is a free escape hatch
> already written into `operator-expected.md` §0/§8: the operator sends the review build on the
> `hard` arm, which shows the disclosure in full. If the fix looks layout-fragile, recommend the
> hatch instead of spending runs on an Architect-gated monetization surface.**
>
