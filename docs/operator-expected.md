...
Nothing below blocks the next session (Session 34 = UIR-2, the dashboard + the
widget families, per YOUR regeneration mandate — roadmap §2.5: the dashboard
moves onto the Session-32 design system and the Session-33 text-scaling rules;
copy bytes untouched; no new SDK, no new key, no operator gate. The widget
screenshot goldens are the session's real cost and get budgeted explicitly at its
planning step. ASO assets/screenshots stay YOURS behind Gate G0 and wait on UIR
(waivable); the FINAL goldens batch still waits on your §3 copy pass and lands
post-UIR as ONE re-record. NOTE: onboarding deliberately minted NO screenshot
goldens this session — they would only be thrown away by your copy pass.)
