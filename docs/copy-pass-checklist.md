...
3. **The legal rider — CLOSED in code.** Terms of Use / Privacy Policy were plain `Text` labels (an Apple Schedule 2 rejection waiting to happen). They are now real `Link`s to `ballast.beyondkaira.com/terms` and `/privacy`, with the URLs as constants in `AppIdentifiers.swift`. **The pages themselves must be published before submission** — that is the one half that left the repo.
