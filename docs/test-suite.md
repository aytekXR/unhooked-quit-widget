...
- **Example scenarios (the full E2E suite):**
  29. `e2e_quizFreshInstall_ageGate_14Steps_summary_paywall_eachStepFiresEvent` — the funnel IS the business (PRD §6.1); asserts `quiz_step_completed(1…14)` → `quiz_completed` → `paywall_viewed` fire in order via a debug event-spy sink. *Shipped (Session 29) as `QuizFunnelUITests.test_quizFunnel_freshInstall_gateToSummary_toPaywallMount` — this slot. "1…14" names the CANONICAL slot DOMAIN (S17-R2); the consent-honest OBSERVED sequence on the driven vape/quit path is `quiz_step_completed(3,4,5,6,7,8,9,10,11,13)` → `quiz_completed` → `paywall_viewed`: slots 1–2 + `onboarding_started` fire pre-consent and are gate-swallowed BY DESIGN (S19-R1/R2 — the measurable funnel begins at the slot-3 opt-in), slot 2 (customName) and slot 12 (allowance) are conditional-hidden on this path, and 14 (summary) fires `quiz_completed`, not a step event. The spy is the consent-honest `DebugEventSpySink` sink-tier decorator (R25.9/R29.5), read through the `debug.eventSpy` a11y bridge under `UITEST_EVENT_SPY=1`.*
  ...
  33. `e2e_voiceOverCompletesQuizPanicAndSlipFlows` — accessibility release criterion driven with the accessibility audit API (`XCUIApplication.performAccessibilityAudit(for:_:)` — the API lives on XCUIApplication, not XCUIDevice; corrected Session 28). *Shipped (E9.3, Session 28) as the per-leg family `test_a11yAudit_{panicFlow,slipFlow,quizFlow}_noViolations` in `Tests/UITests/A11yAuditUITests.swift` — ONE slot (this one); panic/slip legs are §7-rule-11 safety legs; audit scope per R28.13 (grow-only class exclusions, findings ledger = CI run 29262073722's artifact).* **S61 adds an AX5 half to the same family and the same slot** — `test_a11yAudit_{ageGate,quizFlow,summary,paywall,resources}_ax5_noViolations`, because every leg above mounts at the DEFAULT content size and that single blind spot produced R58.1, R58.2, R60.1 and R60.2. **There is no XCUITest API for content size** (docs-JSON-verified: nothing on `XCUIApplication`, `XCUIDevice` or `XCUISystem`), so the size rides UIKit's `-UIPreferredContentSizeCategoryName` argument domain via `launchArguments` — a BEHAVIOUR, not a published API, which is why `test_ax5Override_actuallyTakes_orEveryAX5LegIsAFalseGreen` measures a real element at both sizes and gates the rest: a silent no-op would render every AX5 leg at the default size and PASS, which on an audit lane is worse than having no leg. Scope is the surfaces that pin an action zone, plus resources (the one surface that was blind at AX5 in both lanes at once).
