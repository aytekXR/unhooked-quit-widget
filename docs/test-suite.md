...
- **Example test cases (concrete names):**
  ...
  7. `momentumNeverDecreasesAcrossAnyOperationSequence` — property test: random sequences of slips/undos/time advances; asserts `bestStreakSeconds` and `totalCleanSeconds` are monotonically non-decreasing (Architecture §9 rule 3: "any code path that would decrease them asserts in debug").
  ...
  10. `analyticsEventSlipLoggedCarriesOnlyHabitCategory_noTimestampRepresentable` — compile-time-adjacent test: serializes every `AnalyticsEvent` case and asserts the property whitelist per MVP §5 (no timestamps, no content, no custom names). This is the executable form of the privacy promise.
...
- The automated payload audit (test 27): scripted full session → intercepted request bodies contain **only** the 18 event names and per-event properties from MVP §5; forbidden-key scan (`note`, `text`, `customName`, `at`, `birthYear`, raw timestamps in properties) finds zero hits; `slip_logged` carries `habit_category` and nothing else.
- Opt-out default: fresh install with analytics screen skipped produces **zero** outbound TelemetryDeck requests across a full session (opt-in defaults OFF, Architecture §10).
- `erase_all_confirmed` is the final event: no event fires after erase in the same process lifetime.
