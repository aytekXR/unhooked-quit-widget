...
**Widget refresh:** two independent paths, deliberately not conflated. The **freshness** path is push-based (`reloadTimelines` on every write), which is what satisfies the ≤60s-after-an-event requirement — real-world freshness is seconds. The **rollover** path is the timeline itself: because streak durations self-tick via date-relative text (`Text(timerInterval:)`), entries are needed only where a *number the ticker cannot derive* changes — the day boundary and milestone crossings (the WidgetToolkit pattern) — which stays far under WidgetKit's refresh budget. A user who logs nothing produces no write, so the rollover path must stand on its own. E6.1 ships the **day-boundary half** (local midnight in the quit's fixed zone — ADR-11); **milestone-crossing entries are owed by E6.2**, and are a separate concern precisely because milestones are elapsed-hours-keyed (`afterHours`) and so almost never coincide with a local midnight. Discreet variants are render-time branches, not separate timelines.
...
### ADR-11: Displayed "Day N" is a calendar day in the quit's fixed timezone (E6.1)
**Context:** Two different numbers can both be called "the streak," and until E6.1 no surface rendered either, so the ambiguity was invisible. StreakEngine's `StreakValue.days` is `elapsedSeconds / 86_400` — a **duration** readout, timezone-invariant by construction (ADR-7), 0-based on the quit day, and the input to milestone math (`afterHours`). But the widget must answer the question the user's wall clock asks: a user who quits on Tuesday evening expects to see "Day 2" on Wednesday morning, not 15 hours later. Both plan and test-suite already assumed the calendar reading (`test_timeline_entriesCrossMidnight_incrementDay`, §11's "entries only at midnight boundaries", test-suite §1 scenario 23, and test-suite §1.1's "day boundaries computed in the quit's timezone").

**Decision:** The **displayed** day number is a **1-based calendar day count that increments at local midnight in the quit's FIXED start timezone** — never `TimeZone.current`. The quit day is Day 1 from its first second. `StreakValue.days` remains the duration readout and keeps its zero display consumers; the two numbers are deliberately different and must not be reconciled. Timezone is an **unguarded** input (a two-tap Settings change that ADR-7's monotonic guard cannot see), so pinning it to the quit's own zone is what keeps the count travel-immune (MVP AC#13); reading the device zone would let a westward flight mint a free day. The count is computed by anchoring both endpoints at **noon** of their local day: `startOfDay` is wrong in zones that spring forward *at* midnight (America/Santiago, America/Havana — local 00:00 does not exist, and a quit started that day reads one day low forever), and `Calendar.ordinality(of:.day,in:.era)` is off by one on exact-midnight instants under Linux Foundation, which is every boundary entry.

**Consequences:** **Binding on every future surface** — the dashboard (E9), StandBy, and any Live Activity (E11.1) must render "Day N" by this rule, or two surfaces will disagree by up to a day for the same quit. Rendering `StreakValue.days` as "Day N" is a **bug** (it prints "Day 0" on the quit day). Milestones stay elapsed-hours-keyed and therefore do **not** land on the boundaries the widget renders — an `afterHours: 24` milestone unlocks 24 real hours in, which may be a day after the widget flips to Day 2; E6.2 owes milestone-crossing timeline entries for the systemMedium milestone bar, because a crossing is almost never a local midnight. **Operator-vetoable:** the alternative (Day N = whole 24-hour blocks, matching `StreakValue.days`) is a one-function change plus two test literals.
