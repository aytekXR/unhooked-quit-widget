...
| E7.1–E10.2 (rest) | ☐ not started | Per plan; no scope changes. Next: E7.1 (Session 23 — PaywallKit entitlement state machine [PKG:PaywallKit]; the Linux-lane protocol seam is the load-bearing step-0: RevenueCat is Darwin-only, the adapter lives app-side per ADR-4) |

**Completion ratio (v1.0 scope, E0–E10 = 32 tasks):** 22 done + 1 half-scaffolded
(E0.3: harness shipped, device measurement operator-owned) = **69%** of build
tasks (delivery counter: 24/32 = 75% counting the half); milestones: M0 ✅,
