...
| E6.1–E10.2 (rest) | ☐ not started | Per plan; no scope changes. Next: E6.1 (Session 20 — WidgetToolkit timeline provider: midnight/DST rollover, stale-grace, `Text(timerInterval:)` ticking counters; the `widget-state.json` writer seam exists in `rebuildSnapshots()`) |

**Completion ratio (v1.0 scope, E0–E10 = 32 tasks):** 20 done + 1 half-scaffolded
(E0.3: harness shipped, device measurement operator-owned) = **63%** of build
tasks (delivery counter: 21/32 = 66% counting the half); milestones: M0 ✅,
