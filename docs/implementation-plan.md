...
| E7.3 | ✅ build half DONE (S26) | Win-back: app-side lapse stamp + promotional-offer mechanics + dismissible offer surfaces + fires, all dormant; sandbox matrix = operator tier |
| E9.1 resources + helplines + alcohol notice | ✅ DONE (Session 27) | Red evidence CI `29245297054` (4 designed-failing / 8 issues NAME-FOR-NAME — the 13th consecutive harness-predicted red — + the 2 DESIGNED logged-stage golden shifts / 6 image issues inside the pre-worded 4–8 fold valve) → green `cf75ba6` CI `29246823045` all-green + TestFlight; **exactly 2 billed runs, zero burned — the golden-rides-red maneuver (R27.12) recovered the 6 shifted goldens from the red run's own artifact.** The five plan-named tests VERBATIM: sibling `SafetyResourcesView` (STORE-FREE: bundled JSON + injected analytics — the cold slip route's contract depends on it) one tap from Settings (`settings.resources.row`, the SECOND R22.7 amendment, UNCONDITIONAL/never entitlement-gated) and from every slip flow (`slip.resources.link` on the logged stage, internal sheet, both routes; the cold route's open is intentionally unmeasured — the panic-descended surface constructs no analytics); pure `SafetyResourcesSelection` (unmapped→GLOBAL; verified-only, crisis-first, NO quit filtering) beside the byte-frozen E5.1 funcs (unmapped→US, zero-fire — all S16 pins held); the NEW number-FREE `GLOBAL` region (calm local-emergency line + the live-verified findahelpline.com pointer; a URL-only Helpline row CANNOT decode — one bad row nils the whole directory, so the bucket ships `resources: []` BY CONSTRUCTION); the alcohol withdrawal notice decoded from safetyCopy.json (optional section, `#require`d into the lexicon walk; fail-SAFE degraded fallback) shown ONCE ever app-wide (both goal modes) as an inline amber dashboard card ("Got it" ≥ prominence; `lifepreserver` — no new symbol) stamped via `AppSettings.alcoholNoticeShownAt: Date?` (the lapseObservedAt twin; exact-set pin swept same-commit 8→9; erase-swept); `resources_viewed(source)` live — once per PRESENTATION, consent-gated, source injected by the mount, domain CLOSED {settings, slip_flow} (the notice hand-off passes nil and fires nothing BY RULING R27.4 — no mvp §5 edit). **RECORDED ADJUSTMENTS (vetoable, ledger S27):** dashboard mount ⇒ a live-era hard-walled non-converter never meets the notice; region picker deferred (`regionPickerLabel` ships unconsumed); helpline rows are NEVER lexicon-scanned (sourced material; the SAMHSA "treatment referral" false-positive was burn-reproduced) — the new SafetyCopy gate scans AUTHORED framing only, and `notMedicalCareDisclaimer` reworded treatment→care (§3). Operator half carried: helpline number verification (ALO 182 flag), clinician+counsel copy pass, the §7 signature. |
| E9.2 content-table audit (milestones) | ✅ DONE (build half, Session 27 — batched IN per R27.13; audit SIGNATURE = operator §3) | Both plan-named tests landed as BORN-GREEN permanent CI gates in the same red commit (zero designed-red risk, zero extra runs — scratch-verified over the shipping file first): `test_milestonesJSON_matchesSchema` (JSONSerialization key-SET; exact 6-category set incl. custom; exact row key-sets; 43 rows) + `test_milestoneCopy_containsNoMedicalClaimLexicon` (PHRASE-ANCHORED medical lexicon — bare experiential vocabulary is SANCTIONED under "commonly reported" framing; a naive bare token ("breathing") was burn-REPRODUCED false-firing on the shipping vape table; positive/negative matcher pins + the E4.2-shape foundation-floor only-grows pin + the ≥40 corpus floor + the 43/43 framing sub-pin keep the gate non-vacuous). NO production Milestones model (PM fence) — the tests read the bundled shipping JSON (§3.2). Epic 9 DoD's "banned-lexicon tests permanent in CI" row: CLOSED. The human nuance audit + signature = operator (§3, the E4.2 shape). |
| E9.3 accessibility pass | ✅ DONE (Session 28) | Red evidence CI `29259860083` (3 designed-failing / 3 issues NAME-FOR-NAME with each predicted failure string VERBATIM — the 14th consecutive harness-predicted red — + the designed golden family at 2 image issues BELOW the 3–4 valve: the AX5 axes diffed while the DEFAULT axes passed WITHIN the 1%/0.98 tolerance, the new tolerance-floor house fact) → green across `4bc8829`/`b80c33d`/`2346cba`/`3b6d692`, final run `29267286952` all-green + TestFlight; **5 billed runs = 2 planned + contingency + 2 over, 1 BURNED** (run 3's macOS-only audit members — docs existence ≠ platform availability, now standing gate #5b; both golden re-record batches extracted FREE from CI's own artifacts — R27.12 twice). The three plan-named tests delivered: `test_hapticsOnlyPacer_runsWithoutVisualDependency` VERBATIM (end-to-end: setter → pre-cache ENVELOPE `PanicSnapshot.hapticOnlyBreathPacer: Bool?` presence-only stamp → production-init threading → model; rhythm sub-asserts view-free) + the stamp/threading pins; `test_a11yAudit_quizPanicSlip_noViolations` as the per-leg FAMILY on scenario 33's slot (panic/slip = rule-11 safety legs; quiz via the two-level DEBUG `UITEST_QUIZ` direct mount — pure composition, the scenario-29 wall untouched; scope = iOS members MINUS {contrast, dynamicType, textClipped} — the R28.13 grow-only debt, findings enumerated in run `29262073722`'s artifact); the AX5 truncation row = streak half ALREADY SHIPPED (widget/slip/panic AX5 axes), paywall half DEFERRED BY NAME to the founder-copy golden batch (R24.1 extended incl. AX5 axes + the R28.13 contrast/textClipped visual pass — colors+copy+goldens in ONE re-record). The toggle = the THIRD R22.7 amendment ("Breathe with taps" + footer, DRAFT §3); the VO label/trait pass (slider word-echo value, stepper/fields, icon `.isSelected`, undo hint); `instructionNonVisual` (R28.4, joint-signed) replaces the literally-false "Follow the circle" in haptics-only mode (pixel bytes rode RED per R27.12). The audit's first run FIXED two real defects: the forgiveness screen's dangling "momentum is still ." (`SlipLoggedComposition` — an unfilled token drops its WHOLE sentence; filled path byte-identical, harness-proven) and the 4pt `quiz.progress` hit-region sliver; the title's VO label drops its typographic period (the classifier's one not-human-readable node). Born-green: the panic-script lexicon gate (35-string Mirror corpus — closed a real scan hole). **Epic 9's build half: CLOSED.** Operator half carried: §3 a11y copy block + clinician/counsel pass; the §7 eyes-free eyeball; device test 40. |
| E10.1–E10.2 (rest) | ☐ not started (E10.2 = Session 30) | Per plan. Session 29 (the named StoreKit-config/contract session) CLOSED the carried debts: scenario-29 re-landed GREEN first-run on surfacing anchors incl. the event-spy tail (`QuizFunnelUITests` + `DebugEventSpySink`/`debug.eventSpy` bridge, UITEST_EVENT_SPY-armed); the signed win-back purchase seam BUILT dormant (`purchaseWinback()` + the `winback_annual` adHocOffer in Ballast.storekit, parse/contract-pinned) — the LIVE signed authorization is operator-IAP-key-gated (docs-verified unexercisable keyless, R29.6). Next: Session 30 = E10.2 submission-package prep (build-side half); E10.1 external beta runs on the operator's clock; the goldens batch waits on the operator §3 copy pass |

**Completion ratio (v1.0 scope, E0–E10 = 32 tasks):** 29 done + 1 half-scaffolded
(E0.3: harness shipped, device measurement operator-owned) = **90.6%** of build
tasks (delivery counter: 31/32 = 96.9% counting the half); milestones: M0 ✅,
...
### E7.3 Win-back offer (config) — ✅ build half DONE (Session 26)
- **Goal:** 50%-off annual win-back offered 7 days post trial-lapse via RevenueCat offer, no push dependency.
- **Failing tests first:** `test_winback_eligibility_trialLapsedPlus7Days()`, `test_winback_notShownToActiveOrNeverTrialed()`, `test_winbackPurchase_firesPurchaseWinbackAnnual()`. *(All three shipped VERBATIM, Session 26, plus the eight the panel forced.)*
- **Acceptance:** eligibility verified in sandbox time-travel; surfaced in-app (settings/paywall source), never via notification.
- **Deps:** E7.1.
- **Scope (Session 26 rulings R26.1–R26.15):** eligibility = an app-side observed-lapse stamp + the pure 7-day-duration policy (Apple win-back offers are months-granular + paid-history-gated — docs-verified REJECTED; the mechanism is an **ASC promotional offer**, pay-up-front 1 yr $14.99 on the SAME `ballast.annual` SKU, id `winback_annual`); surfaces = the dismissible OFFER at the reentry gate (entitled > winback > teaser-expiry, once per process) + the eligible-only settings row; fires = winback_shown + paywall_viewed(winback) / winback_converted + purchase. DORMANT by construction. **Sandbox time-travel + the ASC offer + the In-App Purchase Key = operator tier (§8)**; the live signed-offer purchase call rides the named StoreKit-config/contract session.
