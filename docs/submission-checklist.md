...
| No account creation path exists anywhere | **[M-by-absence + M-lint]** code-absence verified S30 (only the two 3.1.1 "Apple Account" billing strings; no auth import/UI) + `PaywallModelTests.test_paywallModel_restoreRecoversEntitlement_unlocks` (account-free restore). **NOW GATED (S41):** the free-Linux `account-absence-lint` CI job fails the build if any shipping source (App/Sources, Shared/Sources, Widgets/Sources) imports `AuthenticationServices` or references `ASAuthorizationController`/`ASAuthorizationAppleIDProvider`/`SignInWithAppleButton`/`ASWebAuthenticationSession` — born-green, and wired into the TestFlight gate's `needs`. The earlier "no standing grep gate" flag is CLOSED. |
...
| Accessibility pass: Dynamic Type, VoiceOver quiz+panic, haptics-only pacer | **[M]** UI `A11yAuditUITests` — **11 surfaces on Apple's full 7-type audit, plus (S61) 5 of them re-run at AX5** (`_ax5_` legs, gated by `test_ax5Override_actuallyTakes_…` so a silent no-op cannot report itself green) + haptics-only pacer tests + **167 goldens across 16 suites, 15 of them carrying the light/dark/light-ax5/dark-ax5 matrix** · **⚠️ THE STATED REASON THIS BOX WAS UNCHECKED IS OBSOLETE AND IS CORRECTED HERE, NOT TICKED:** it read *"the R28.13 contrast/text-scaling exclusions wait on the §3 founder-copy golden batch (ONE re-record)"* — the exclusion list has been **ZERO since UIR-3**, the §3 copy pass closed in 46B, and **the golden batch COMPLETED in S60**. All three blockers are gone. · **[O]** the box nevertheless stays **honestly UNCHECKED** on R30.1 (no agent ticks a §7 box) and on two real remainders: the eyes-free/VoiceOver eyeball is §7 device work, and **R61.1 is open** — two AX5 frames where Apple's `.contrast` check disagrees with a visually-verified golden, cause not yet established, those two audit calls deferred rather than suppressed |
