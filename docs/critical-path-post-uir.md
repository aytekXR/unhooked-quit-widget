...
3. **Open the EXTERNAL beta** (step 10) — third on this list for the same reason as 1 and 2: **testers run on their own clock**, the gate wants ≥1 week of data, and every day it is not started is a day added to launch. **S59 built the ring and filled everything Apple asks for**: the external group `Friends (external)` (`8b856317-1da2-4c41-804e-3299349951f3`, public link off), the Beta App Description, the feedback email, the privacy-policy + marketing URLs, and the newest build's "What to Test" are all written on the live account, and nothing has been sent to Apple. **What is left is four steps in `operator-expected.md` §5, and the first is item 1½ below rather than anything TestFlight-shaped.** Read `testflight-beta-kit.md` §0.1 first. *(This item used to say the internal `Friends` group needed nothing but testers. It cannot host friends at all — internal groups take only Users on your ASC account, of which there is exactly one.)*
   **1½. Deploy `ballast.beyondkaira.com`** (~30 min, your server) — it moved ahead of the beta because the beta's own Test Information now declares the privacy-policy URL, and the paywall's two legal links fail at TLS until certbot issues for the subdomain. `docs/public-site-deploy.md` is paste-ready; `scripts/verify_public_site.sh` says when it is done.
