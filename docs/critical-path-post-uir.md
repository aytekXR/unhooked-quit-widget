...
| Field | Value |
|---|---|
| Document | Post-UIR critical path (created Session 41, 2026-07-19; synced Session 42; synced Session 43 — a 16-agent runbook-fidelity sweep corrected step 7's G0 overstatement (the domain + bundle identity are already registered; only trademark/name clearance is open) and its broken `roadmap.md §Naming` anchor; **re-confirmed accurate Session 44, 2026-07-22** — no operator commits since S43, CI still green, no step here changed; **re-confirmed again Session 45, 2026-07-24** — still no operator commits (HEAD `504fb40`), CI last code run green on all 10 jobs, a first-hand local free-lane run passed 121 tests, no step here changed) |
| Status | LIVE — the single-page "what do I do next, in what order" for the operator. Everything here is human/operator/device/legal work; the one agent item left is the **R46.2 rider on step 4**, which is gated on your RevenueCat key. |
| ⛔ Session 48 — ONE DECISION BLOCKS THE ONLY REMAINING AGENT TASK | The §3 copy pass unblocked the **final golden batch**, and S48 scoped it end-to-end and was ready to mint. It stopped, because the **`redesign/design-roadmap.md`** you committed in the same session schedules changes to **all four** surfaces that batch covers, before launch: QW-6 (crest on the age gate, Phase 2), ME-8 (the waterline field behind the quiz + warmer spend/custom-name steps, Phase 3), **ME-4 "Summary payoff redesign"** and **ME-9 "Paywall goldens + reachable polish"** (Phase 4) — and that roadmap says goldens "re-record once" at Phase 4 so screenshots "shoot against final UI", with Phases 1–4 framed as "to launch-ready … ~7 weeks". Minting now would spend 2 billed runs on goldens re-recorded within weeks. Minting later delays screenshots, which gate submission. **Only you can say which document governs — see `operator-expected.md` §0 (three options, two minutes).** The full mint plan is scoped and banked, so answering (A) or (C) starts it immediately with no re-scoping. Everything else on this page is unchanged and unaffected. |
...
| 4 | **§8 keys** — paste in order: **RevenueCat** (wakes monetization) → **Superwall** (A/B) → create **ASC products + the win-back promotional offer + upload the In-App Purchase Key** → **TelemetryDeck app ID** (+ salt decision). **⚠️ Carries the R46.2 agent rider** — one agent run to land the entitlement-refresh fix, best done WITH the RC key and BEFORE the sandbox matrix (see §8's first bullet) | [K] + [A] | the sandbox matrix; the payload audit; live funnel data | `operator-expected.md` §8 |
